...
int main(int argc, char** argv) {
    ...
    char originalParagraph[MAX_PARAGRAPH_LENGTH];
    if (fgets(originalParagraph, MAX_PARAGRAPH_LENGTH, file) == NULL) {
        perror("Error reading paragraph");
        fclose(file);
    }

    char processedParagraph[MAX_PARAGRAPH_LENGTH];
    strcpy(processedParagraph, originalParagraph);

    makeLower(processedParagraph);
    removePunct(processedParagraph);

    char *originalWords[MAX_PARAGRAPH_LENGTH / 2];
    int originalWordCount = 0;
    char *token = strtok(originalParagraph, " \n");
    while (token != NULL) {
        originalWords[originalWordCount++] = token;
        token = strtok(NULL, " \n");
    }
    ...
    int wordCount = 0;
    token = strtok(processedParagraph, " \n");
    while (token != NULL) {
